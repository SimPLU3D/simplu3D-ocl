...
import fr.ign.cogit.simplu3d.rjmcmc.cuboid.configuration.GraphConfigurationPredicate;
...
public class OCLBuildingsCuboidFinalWithPredicate {
	...
	public OCLBuildingsCuboidFinalWithPredicate() {
	}
	...
	public static ModelInstanceGraphConfiguration<Cuboid> create_configuration(SimpluParameters p, Geometry geom,
			BasicPropertyUnit bpu, UrbaZoneOCL uz) {
		...
		// u=energyCreation-pVolume*energyVolume+pDiff*diffVolume
		...
		// b=pVolumeInter*IntersectionVolume
		BinaryEnergy<Cuboid, Cuboid> binaryEnergy = new MultipliesBinaryEnergy<Cuboid, Cuboid>(c3, b1);
		// empty initial configuration*/
		GraphConfigurationPredicate<Cuboid> pred = new GraphConfigurationPredicate<Cuboid>(bpu, uz);
		ModelInstanceGraphConfiguration<Cuboid> conf = new GraphConfigurationWithPredicate<Cuboid>(bpu, pred,
	...
	static Sampler<ModelInstanceGraphConfiguration<Cuboid>, ModelInstanceModification<Cuboid>> create_sampler(
		...
		// UniformBirthInGeom<Cuboid2>(new Cuboid2(env.minX(),
	...
	// private static <T extends SimpleObject> Kernel<T>
