 ...
 * @version 1.0
 * 
 *          Classe permettant de générer et d'exporter un ensemble de boîtes
 *          suivant la distribution paramétrée
 * 
 * 
 * 
 */
public class DistributionGeneration {
	...
	public static void main(String[] args) throws Exception {
		...
		Parameters p = Parameters
				.unmarshall(new File("./src/main/resources/scenario/building_parameters_project_expthese_1.xml"));
		...
		ModelInstanceGraphConfiguration<Cuboid> config = new ModelInstanceGraphConfiguration<>(bpu,
				pred.getRuleChecker().getlModeInstance().get(0), new ConstantEnergy<Cuboid, Cuboid>(0),
				new ConstantEnergy<Cuboid, Cuboid>(0));

		Sampler<ModelInstanceGraphConfiguration<Cuboid>, ModelInstanceModification<Cuboid>> samp = create_sampler(p,
				bpu, pred);
	...
	static Sampler<ModelInstanceGraphConfiguration<Cuboid>, ModelInstanceModification<Cuboid>> create_sampler(
			Parameters p, BasicPropertyUnit bpU, ModelInstanceGraphConfigurationPredicate<Cuboid> pred) {
		...
		// Math.PI), builder,
		// bpU.getpol2D());
		UniformBirth<Cuboid> birth = new UniformBirth<Cuboid>(rng, new Cuboid(0, 0, mindim, mindim, minheight, 0),
				new Cuboid(1, 1, maxdim, maxdim, maxheight, Math.PI), builder, TransformToSurface.class,
				bpU.generateGeom());
		...
		Sampler<ModelInstanceGraphConfiguration<Cuboid>, ModelInstanceModification<Cuboid>> s = new GreenSampler<>(rng,
