 ...
 * 
 * This file is part of the Java Model Instance Plug-in of Dresden OCL2 for
 * Eclipse.
 * 
...
public class JavaModelInstanceFactory extends BasisJavaModelInstanceFactory
  ...
  public Map<Object, IModelInstanceElement> getMyCachedAdaptedObjects() {
    return myCachedAdaptedObjects;
  }
   ...
   * @throws TypeNotFoundInModelException Thrown, if a given {@link Enum} cannot
   *           be adapted to a {@link Type} in the {@link IModel}.
   */
   ...
   * @throws TypeNotFoundInModelException Thrown, if a given {@link Object}
   *           cannot be adapted to a {@link Type} in the {@link IModel}.
   */
   ...
   * 
   * @param object The {@link Object} that shall be adapted.
   * @param The {@link Type} the created {@link IModelInstanceObject} shall
      ...
      try {
        ...
        if (typeClass.isAssignableFrom(object.getClass())) {
          ...
          try {
            ...
            result = new JavaModelInstanceObject(object, typeClass, type,
                originalType, this);
          }

          catch (TypeNotFoundInModelException e) {
            /* Create an undefined instance object. */
            result = new JavaModelInstanceObject(null, typeClass, type, type,
                this);
          }
        }
   ...
   * </p>
   * 
   * @param aClass The {@link Class} for that the {@link Type} shall be
   *          returned.
  ...
  private HashMap<Class<?>, Set<Type>> linksType = new HashMap<>();
   ...
   * </ul>
   * 
   * @param aClass The {@link Class} for that the {@link Type}s shall be
   *          returned.
  ...
  private Set<Type> findTypesOfClassInModel(Class<?> clazz)
    ...
    if (clazz != null) {
      ...
      Set<Type> resCache = linksType.get(clazz);
      if (resCache != null) {
        return resCache;
      }
    ...
    if (result.size() == 0) {
      ...
      this.linksType.put(clazz, result);
    }
   ...
   * </p>
   * 
   * @param object The {@link Object} for which the {@link Type} shall be
   *          returned.
   * @return The found {@link Type}.
   * @throws TypeNotFoundInModelException Thrown, if a given {@link Object}
   *           cannot be adapted to a {@link Type} in the {@link IModel}.
   ...
   * </p>
   * 
   * @param types The {@link Set} from which super {@link Type}s shall be
   *          removed.
